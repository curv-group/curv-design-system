...
2. Copy `node_modules/@curvgroup/design-system/examples/<shell>.tsx`. One fitting job: `dashboard-page.tsx` (no tabs). Two named jobs: `dashboard-page-tabs.tsx`. Chart + table + row peek: `report-page-performance.tsx`. Wire real data. Do not add sections.
3. Tabs only when there is more than one job or a slot overflows. Count jobs — do not default to three tabs. Extra data goes in a tab, a drawer, or a hover — never a wall of cards.
