...
| A side panel (details, forms) | `Drawer`, `DrawerClose`, `DrawerSection`, `DrawerRow` | `trigger`, `side`, `size`, `title`, `description`, `badge`, `headerActions`, `footer`, `open`/`onOpenChange`. `DrawerRow`: optional `icon`, label, left-aligned value. Footer: Close left, primary right. Copy `examples/drawer.tsx` |
